...
## 硬件图层 overlay 框架(2026-07-20;2026-08-04 已上真车)
...
- **状态**:已在真车上运行。半透明 Material 风格面板 + 真 8 位 alpha 平面(软圆角、elevation 投影),
  音量条跟着旋钮实时走;而花时间最多的一块 —— 与倒车雷达、倒车影像的显示正确共存 —— 也已解决。
- **最重要的一条教训**:硬件层你永远不可能独占。系统没有归属权机制,逐层状态是同一份共享记录、后写者赢。
  去找"一块没人用的层"是死路,因为**层分布随车型而变** —— 在 Panamera 台架上 221 秒重度操作零变化的那块层,
  在 911 上正是 PDC 雷达层。正解是**让出协议**:每拍监视自己那条记录,一旦原厂开始用它,就**彻底停手**,
  直到它重新空闲。这照搬了原厂自己的优先级模型,而且不需要知道任何车型的层分布。详见文档 §2.6。
