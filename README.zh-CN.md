...
**Porsche PCM3.1 —— 两个已上车的改装、一项台架能力,以及做出它们的那套工具箱**

> Porsche PCM3.1(CHN)· QNX 6.3.x · SH-4A · 2026-08
> **✅ Feature 1 和 2 已在真车(911/9x1)上运行,不只是台架。**
>
...
| **3 · [USB 网卡](usb-ethernet.zh-CN.md)** | 用固件白名单不认的 AX88772B,把网线接到台架上 | 驱动从硬盘加载,**零 flash 写入** | ✅ 台架 |
...
三份 feature 文档用的是同一套骨架,所以你读哪一份都是同一个读法:
...
# Feature 3 · USB 网卡 —— 让白名单不认的卡跑起来

📄 完整文档:**[usb-ethernet.zh-CN.md](usb-ethernet.zh-CN.md)** · [English](usb-ethernet.md)

台架唯一的调试通道是 **57600 波特串口**。固件其实早就为另一条路准备好了 —— `inetd.conf`
里 ftp 和 telnet 都是 root 起的 —— 但绑定 USB 网卡要**精确 VID/PID 对上那四条中的一条**,
而台架上这张卡是 AX88772B(`0b95:772b`),不在里面。2026-07 的结论是"买一张原生对得上的卡,
别啃 772B",这条结论管了六个星期。

**台架上的最终效果**:就用这张被拒的卡,`en0` 起来了,192.168.0.90,链路已协商。

## 技术解释

- **两个互不相干的故障叠在一起**,外面那个把里面那个盖得严严实实。
- **外面那个** —— `mount -Tio-net devn-asix.so` 不报错,它把 `io-net` 直接打成 **SIGSEGV**,
  试三次,每次同一个 `ip`。成因和这张卡毫无关系:`devn-asix.so` 的 `DT_NEEDED` **只有
  `libc.so.2`**,却引用了一整组 USB DDK 符号(`udi_attach`、`udi_io`、`udi_enumerate`……)。
  它从来没被逼着声明这些依赖:原厂的加载路径会把 DDK 一起带上。换任何别的方式加载,第一次
  经由未解析符号发起调用就跳野地址。
- **里面那个** —— DDK 喂上之后,驱动能正常初始化,然后在业务层拒绝:
  `unable to init dll devn-asix: No such device`。这是驱动自己的芯片表 —— 模块内部的
  **第二份白名单** —— 里面同样没有 `0b95:772b`。
- **把两者分开的是对照组。** 拿*没改过*的驱动挂一次、看着它以同一个 `ip` 崩掉,只要一分钟,
  而这就是全部的诀窍:先去改 PID 表的话,改完照样以同样的方式崩,于是一个正确的修法看起来
  就像一个失败的修法。

## 技术方案

一个问题一行修法,而且**全程不写 flash** —— 打好补丁的驱动就是硬盘上一个普通文件,挂进一个
你自己起、也能自己杀掉的第二个 `io-net` 实例。

- **`LD_PRELOAD=/lib/libusbdi.so.2`** 把 DDK 带进第二个实例(`-i1`,`/dev/io-net1`),
  原本在跑的东西一概不动。
- **芯片表改一个字节** —— 文件偏移 `0x107e8`,PID `0x772a` → `0x772b`,把 AX88772**A**
  那条记录改指过去,于是 772B 沿用它的 flags 和芯片标记,不需要新加记录。

| | 位置 |
|---|---|
| **补丁生成器** | [`code/usb-ethernet/patch_asix_pid.py`](code/usb-ethernet/patch_asix_pid.py) —— 按特征搜芯片表(不硬编偏移)、旧值对不上就拒绝、打印改动的每个字节 |
| 设备端字节编辑器 | [`code/usb-ethernet/fx.c`](code/usb-ethernet/fx.c) —— 带"旧值必须对得上"闸的文件字节读/写,改一个字节不用再跑一趟 U 盘 |
| 驱动二进制 | **不含** —— 保时捷专有固件;从你自己的 dump 里抠出 `devn-asix.so` 喂给生成器 |

> 🚨 **台架只有一个 USB 口。** 网卡和 U 盘插不到一起,所以打好补丁的驱动必须**先**落到
> `/mnt/data`(硬盘,持久)上,再把 U 盘换成网卡。

## 涉及的问题

五条,包括真正要命的那条:上来就去改 PID 表 —— 得到的是一个照样崩的驱动和一个错误的结论。
**[→ usb-ethernet.zh-CN.md 第三部分](usb-ethernet.zh-CN.md)** 全都记着,还有
`DT_NEEDED` 这一条到底说明了什么、没说明什么。

---

# 共用工具

三个 feature 都是踩在这些上面长出来的,缺了它们哪个都做不成。
...
usb-ethernet.md    (+ .zh-CN.md)   feature 3 完整文档
code/
  bluetooth-fix/                   cave、离线验证器、USB autorun 刷写脚本
  volume-osd/                      overlay 引擎、渲染器、ui.def、截图验证
  usb-ethernet/                    ASIX 驱动 PID 表补丁生成器、设备端字节编辑器
