...
**面板可绘制高度是 `h - shadow`,不是 `h`**(`ui_render` 里 `ph = H - sh`),宽度同理是
`w - shadow` —— 阴影是从底边和右边抠出去的。

所以要把一个高 `eh` 的元素纵向居中,应该用 `(h - shadow - eh) / 2`,而不是
`(h - eh) / 2`。算错的后果是整体下移 `shadow/2` —— 我们第一版就是这么错的:
`h=76, shadow=12` 时面板实际只有 64 高,但元素坐标是按 76 算的,于是整个布局偏低,
进度条上方的留白差不多是下方的两倍。
